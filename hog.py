...
def select_dice(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    return six_sided if (score + opponent_score) % 7 else four_sided
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < goal and opponent_score < goal:
        if who:
            opponent_score += take_turn(strategy1(opponent_score, score),
                                        score,
                                        select_dice(opponent_score, score))
            score, opponent_score = swine_swap(score, opponent_score)
        else:
            score += take_turn(strategy0(score, opponent_score),
                               opponent_score,
                               select_dice(score, opponent_score))
            opponent_score, score = swine_swap(opponent_score, score)
...
def swap_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    roll_zero = take_turn(0, opponent_score) + score

    # beneficial
    if roll_zero * 2 == opponent_score:
        return 0
...
# additional function
def is_four_sided(score, opponent_score):
    return False if (score + opponent_score) % 7 else True


def bacon_strategy2(score, opponent_score):
    if take_turn(0, opponent_score) >= BACON_MARGIN:
        return 0
...
def swap_strategy2(score, opponent_score):
    roll_zero = take_turn(0, opponent_score) + score
    ...
    return bacon_strategy2(score, opponent_score)


def final_strategy(score, opponent_score):
    """Write a brief description of your final strategy.

    swap_strategy2 and bacon_strategy2 (called by swap_strategy2) uses as 6 BASELINE_NUM_ROLLS.
    the reason is based on the max_scoring test, rolling 6 has higher chance to produce max score.

    Current setting winning rate: 0.59 or higher (90% chance)
                                  0.58 (10% chance)
    """
    "*** YOUR CODE HERE ***"
    free_bacon = take_turn(0, opponent_score)

    if score + free_bacon >= GOAL_SCORE:
        return 0

    if opponent_score > score > 69:
        return BASELINE_NUM_ROLLS + 1

    # make the opponent get a four_sided dice
    # while making sure the free_bacon at least gives 5 points
    if not (score + free_bacon + opponent_score) % 7:
        if free_bacon >= 5:
            return 0

    # four_sided dice has a 25% chance of getting 'pig out'
    # take lower risk by getting free_bacon when it at least gives 5 points
    # otherwise rolls 4 times as it has higher chance to reach
    # max score based on the max_scoring test
    if is_four_sided(score, opponent_score):
        if free_bacon >= 5:
            return 0
        else:
            return BASELINE_NUM_ROLLS - 1

    return swap_strategy2(score, opponent_score)
